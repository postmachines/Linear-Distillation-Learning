...
def test_predictors(model, data_loader, device, test_batch=1000, silent=True,):
    ...
    with torch.no_grad():
        ...
        for batch_i, (x, y) in enumerate(data_loader):
            ...
            mses_tensor = torch.Tensor(2, test_batch).to(device)
            torch.cat(mses, out=mses_tensor)
            mses_tensor = mses_tensor.view(2, test_batch)
...
def run_experiment(config):
    ...
    accs = []
    dataloader = get_episodic_loader(dataset, 2, train_shot, test_shot, 41,
                                     split=split)
    ...
    for i_trial in tqdm(range(trials)):
        ...
        for sample in dataloader:
            ...
            model = BidirDistill(n_classes=2,
                    in_dim=41,
                    out_dim=41,
...
#             results_data += train_trial_data
            test_acc = test_predictors(model, samples_test, silent=silent, device=device, test_batch=test_batch)
