    ...
    with torch.no_grad():
        ...
        for batch_i, (x, y) in enumerate(test_loader):
            ...
            for predict in predict_next_state_feature:
                cos_distances.append(torch.abs(cos(target_next_state_feature.reshape(1, -1), predict.reshape(1, -1)))[0])
            class_min_cos_dist = np.argmax(cos_distances)
            if class_min_cos_dist == y.item():
                correct += 1
        acc = correct / n
        if not silent:
